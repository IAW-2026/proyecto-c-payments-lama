    ...
    <main className="lama-home relative grid min-h-screen place-items-center overflow-hidden bg-[#17211f] px-6 text-white">
      ...
      <section className="relative max-w-md rounded-3xl border border-white/12 bg-white/[0.08] p-8 text-center shadow-[0_24px_70px_rgba(0,0,0,0.3)] backdrop-blur-xl">
        ...
        <p className="mt-3 leading-7 text-white/68">
          Para ver tus ventas y cobros necesitás estar logueado.
        </p>
        <Link
          href="/sign-in"
...
export default async function VentasPage({ searchParams }: PageProps) {
  ...
  const ventasFiltradas = filtrarPorPeriodo(
    ventas,
    periodo,
    semanaSeleccionada,
    mesSeleccionado
  ...
  const netoARecibir = ventasFiltradas.reduce(
        ...
        <div className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr] 2xl:grid-cols-[0.74fr_1.26fr]">
          <div className="pt-8 lg:pt-14">
            ...
            <h1 className="lama-compact-heading mt-7 max-w-3xl font-black leading-[0.92] tracking-normal">
              Mis ventas
              <span className="block text-[#a8bba0]">y cobros</span>
            </h1>

            <p className="mt-7 max-w-2xl text-lg font-medium leading-8 text-white/72 sm:text-xl sm:leading-9">
              Revisá cada venta, las comisiones aplicadas y el neto que queda
              para recibir.
            </p>

            <div className="mt-7 grid max-w-xl grid-cols-2 gap-2">
              <MetricGlass label="Ventas" value={ventasFiltradas.length} />
              <MetricGlass
                label="Total"
                value={formatearMonto(totalVendido)}
              />
              <MetricGlass
                label="Neto"
                value={formatearMonto(netoARecibir)}
                accent
              />
              <MetricGlass
                label="Comisiones"
                    ...
                    >
                      ...
                      <div className="mt-3 grid gap-3 md:grid-cols-3">
                        <div className="lama-compact-detail bg-[#f6f1e7]">
                          ...
                          <p className="mt-2 font-black text-[#37413d]">
